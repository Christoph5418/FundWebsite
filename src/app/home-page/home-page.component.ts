...
export class HomePageComponent implements OnInit {
    ...
    constructor( private stockFetchingService: StockFetchingService, private router: Router, private sharedDataStore: DataShareService, private stockPrice: StockPriceService) { 
    ...
    private setTable(): void{
      ...
      for(let i = 0; i < this.rowData1.length; i++){
        if(Math.abs(parseFloat(this.rowData1[i]['Percent Change'])) > moveMuch[0]){
          moveMuch[0] = Math.abs(parseFloat(this.rowData1[i]['Percent Change']));
      ...
      for(let i = 0; i < this.rowData1.length; i++){
        if(moveMuch.includes(Math.abs(parseFloat(this.rowData1[i]['Percent Change'])))){
          this.movers.push(this.rowData1[i])
        }
      }
    ...
    private setLine(): void{
      ...
      for(let a = 0; a < this.historicalData.length; a++){
        tot += this.historicalData[a][0] * this.rowData1[a]['Shares']
  
        for(let b = 0; b < this.historicalData[0].length; b++){
          sumA[b] += (this.historicalData[a][b] - this.historicalData[a][0]) * this.rowData1[a]['Shares']
        }
      }
    ...
    // async fetchStockData(ticker: string): Promise<void> {
    //   try {

...
export class StrategyComponent implements OnInit {
  ...
  final: any[] =[]
  monteCarloChart: Chart | undefined
  frontierChart: Chart | undefined
...
'rgb(134,157,122)', 'rgb(212,180,131)', 'rgb(140,179,105)', 'rgb(110,103,95)', 'rgb(164,36,59)'];
  ...
  ngOnInit(): void {
    ...
    //declaring the graphs as empty
    this.monteCarloChart = new Chart("monteCarlo", {
      type: 'line', // This denotes the type of chart
    
      data: {
        // If you want to show specific labels on the X-axis, specify them here.
        // Otherwise, you can set it to an empty array.
        labels: [], 
        datasets: [] // Empty array for no data sets
      },
      options: {
        scales: {
          x: {
        ...
        elements: {
          // Any element options can be specified here
        },
        aspectRatio: this.aspectRatio // Maintain the aspect ratio you need
      }         
    });

    const ctx = document.getElementById('myChart') as HTMLCanvasElement;

    this.frontierChart = new Chart(ctx, {
      type: 'scatter',
      data: {
          datasets: [] // Set to an empty array for no data sets
      },
      options: {
          plugins: {
              legend: {
          ...
          scales: {
              x: {
                  ...
                  title: {
                      display: true,
                      text: 'Volatility' // Customize as needed
                  }
              },
              y: {
                  title: {
                      display: true,
                      text: 'Returns' // Customize as needed
                  }
              }
          },
          aspectRatio: this.aspectRatio // Set the aspect ratio as needed
      }
  });
  ...
  private displayMonteCarlo(): void{
    ...
    //must create data set of all timelines
    let datasets:any = []
    ...
    //gets array (x-index) of same size
    ...
    //makes sure to update chart
    this.monteCarloChart.update();
  }

  //method to get stock data from flask server
  ...
  async delFrontier(): Promise<void>{
    ...
    //same thing as add but for deleting
    this.errorMessageFrontier = ""
    this.tickerFrontier = this.tickerFrontier.replace(/\./g, '-');
    this.tickerFrontier = this.tickerFrontier.toUpperCase();
  ...
  async frontier(): Promise<void> {
    ...
    if(this.frontierChart){
      this.frontierChart.destroy();
    }
...
//all of these methods are helper methods for calculations and graph set up
